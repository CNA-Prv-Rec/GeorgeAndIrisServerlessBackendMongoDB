...
exports = async function(payload, response) {
 ...
 //   return  "Hello World!";
  ...
  if (payload.body) 
  {
     ...
     // user = EJSON.parse(payload.body.text());
     console.log(payload.body);
     console.log(payload.headers);
  /*
       var user2 = jwt(payload.headers.Authorization);
      console.log("parsed user: " + JSON.stringify(user2));
      var collection = context.services.get("mongodb-atlas").db("GeorgeAndIris").collection("Users");
      const email = user2.email;
      const firstName=user2.name;
      const verified = user2.email_verified;
      
        const query = { email: email};
        
         const existingUser = await movies.findOne(query);
